...
def main(filename, compare_filename=False, crop_timestamp=False, return_all=False):
...
def get_isbn(filename, compare_filename, crop_timestamp, return_all):
  ...
  if return_all:
    ...
    if '.epub' in filename:
      ...
      isbns.update({"PDF ISBNS": extract_isbn_from_pdf(filename, return_all)})
    return isbns
...
def extract_isbn_from_pdf(filename, return_all=False):
  ...
  if text:
    ...
    if return_all:
      ...
      isbns = re.findall(isbn_pattern + pdf_only_pattern, text, re.IGNORECASE)
...
def get_no_pages(filename):
  try:
    ...
    sys.stderr.write("Cannot read (or get number of pages in) file: " +  filename + "\n")
   
def get_pdf_text(filename, range):
  cmd = pdftotext_exec
  # UNIX:
  return Popen(cmd + " -f " + str(range[0]) + " -l " + str(range[1]) + " \"" + filename + "\" -", stdout=PIPE, shell=True).communicate()[0]

if __name__ == "__main__":

  parser = argparse.ArgumentParser(description="ISBN Sniffer", usage="Usage: %s [-c --crop --all] file\n" % sys.argv[0])
  parser.add_argument('input_file', action='store', help="Specify the input file to be sniffed")
  parser.add_argument('--crop', action="store_true", dest="CROP", default=False, help="Crop 10 characters from filename")
  parser.add_argument("-c", action="store_true", dest="COMPARE", default=False, help="Compare filename")
  parser.add_argument("--all", action="store_true", dest="ALL", default=False, help="Return all matches")
